	...
	try:
		for j in [100,120,140,150,160]:
			...
			for k in range(5):
				...
				times.append(tt)
			timings.append(np.array(tt).mean())
			sdev.append(np.std(np.array(tt)))
		state = 1
	except:
		state = 0
	print "State: ", state
...
fh = open('/homes/scollis/load.pickle', 'w')
pickle.dump(([128,64,32,16],timings, sdev), fh)
fh.close()
